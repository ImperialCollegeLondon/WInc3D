module actuator_line_beam_model
    ...
    real(mytype),  allocatable :: pos_dot_def(:,:)                   ! velocities
    real(mytype),  allocatable :: psi_dot_def(:,:,:)                 ! time derivative of CRV
    real(mytype),  allocatable :: pos_ddot_def(:,:)                   ! accelerations
    real(mytype),  allocatable :: psi_ddot_def(:,:,:)                 ! second time derivative of CRV
...
contains
    ...
    allocate(beam%pos_ddot_def(beam%Nnodes,3))
    allocate(beam%psi_ddot_def(beam%Nnodes,3,3))
    ...
    ! TODO: ddot parameters should be computed to properly simulate elasticity
    beam%pos_ddot_def=0.0_mytype
    beam%psi_ddot_def=0.0_mytype
