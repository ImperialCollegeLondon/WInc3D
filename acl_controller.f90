...
real(mytype), parameter :: PC_KK= 0.1099965    ! Pitch angle were the the derivative of the aerodynamic power 
	                                      ! w.r.t. pitch has increased by a factor of two relative to the 
	                                      ! derivative at rated pitch (zero), rad.
real(mytype), parameter :: PC_KP= 0.01882681   ! Proportional gain for pitch controller at rated pitch (zero), sec.
real(mytype), parameter :: PC_MaxPit = 1.570796     ! Maximum pitch setting in pitch controller, rad.
...
real(mytype), parameter :: VS_MaxTq = 47402.91  ! Maximum generator torque in Region 3 (HSS side), N-m. -- chosen to be 10% above VS_RtTq = 43.09355kNm
real(mytype), parameter :: VS_Rgn2K = 2.332287  ! Generator torque constant in Region 2 (HSS side), N-m/(rad/s)^2.
real(mytype), parameter :: VS_Rgn2Sp= 91.21091  ! Transitional generator speed (HSS side) between regions 1 1/2 and 2, rad/s.
real(mytype), parameter :: VS_Rgn3MP= 0.01745329! Minimum pitch angle at which the torque is computed as if we are in region 3 
