module dynstall
    ...
    subroutine dystl_init(ds,dynstallfile)
        ...
        NAMELIST/DynstallParam/Tp, Tf, TAlpha, alphaDS0DiffDeg, r0, Tv, Tvl, B1, B2, eta, E0,&
                               CNAlpha, CD0,alpha1,S1,S2,alphaSS,K1,K2
    ...
    subroutine EvalStaticData(ds)
        ...
        ds%CNAlpha=ds%CNAlphalist(ilow)+(ds%Re-ds%ReList(ilow))*(ds%CNAlphalist(iup)-&
                   ds%CNAlphaList(ilow))/(ds%ReList(iup)-ds%ReList(ilow))
        ds%CD0=ds%CD0List(ilow)+(ds%Re-ds%ReList(ilow))*(ds%CD0List(iup)-ds%CD0List(ilow))/(ds%ReList(iup)-ds%ReList(ilow))
        ds%alpha1=ds%alpha1list(ilow)+(ds%Re-ds%ReList(ilow))*(ds%alpha1list(iup)-&
                  ds%alpha1List(ilow))/(ds%ReList(iup)-ds%ReList(ilow))
        ...
        ds%alphaSS=ds%alphaSSList(ilow)+(ds%Re-ds%ReList(ilow))*(ds%alphaSSlist(iup)-&
                   ds%alphaSSList(ilow))/(ds%ReList(iup)-ds%ReList(ilow))
