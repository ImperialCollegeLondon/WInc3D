...
contains
    ...
    subroutine actuator_line_element_write_output(act_line,dir)
        ...
        write(2017,Format) ielem,',',act_line%PEx(ielem),',',act_line%PEy(ielem),',',act_line%PEz(ielem),',',&
                           act_line%ERdist(ielem)/act_line%L,',',act_line%EAOA(ielem)*180/pi,',',act_line%EAOAdot(ielem)*pi/180,&
                           ',',act_line%ERE(ielem),',',act_line%EUr(ielem),',',act_line%ECL(ielem),',',act_line%ECD(ielem),',',&
                           act_line%ECM(ielem),',',act_line%ECN(ielem),',',act_line%ECT(ielem),',',act_line%EFn(ielem),',',&
                           act_line%EFt(ielem),',',act_line%EEndeffects_factor(ielem)
        end do
        close(2017)
    ...
    subroutine dynamic_stall_write_output(act_line,dir)
        ...
        do ielem=1,act_line%NElem
            write(2018,Format) ielem,',',act_line%ERdist(ielem)/act_line%L,',',act_line%Epitch(ielem)*180/pi,',',&
                               act_line%EAOA(ielem)*180/pi,',',act_line%EDynstall(ielem)%fprime
        end do
        close(2018)
    ...
    subroutine actuator_line_turbine_write_output(turbine,dir)
        ...
        write(2016,*) 'Number of Revs, GeneratorSpeed, GeneratorTorque, BladePitch1, BladePitch2, BladePitch3, Omega, DOmega, &
                       &Ux, Uy, Uz, Thrust, Torque, Power'
        write(2016,*) '[-], [rad/s], [N m], [deg], [deg], [deg], [rad/s], [rad/s], [m/s], [m/s], [m/s], [N], [N m], [W]'
        Format="(14(E14.7,A))"
        write(2016,Format) turbine%AzimAngle/(2*pi),',',turbine%controller%GenSpeed,',',turbine%controller%GenTrq,',',&
                           turbine%controller%PitCom(1)*180.0_mytype/pi,',',turbine%controller%PitCom(2)*180.0_mytype/pi,',',&
                           turbine%controller%PitCom(3)*180.0_mytype/pi,',',turbine%angularVel,',',turbine%deltaOmega,',',&
                           turbine%Ux_upstream,',',turbine%Uy_upstream,',',turbine%Uz_upstream,',',turbine%Thrust,',',&
                           turbine%Torque,',',turbine%Power
        close(2016)
