...
contains
    ...
    call QuatRot(turbine%blade(iblade)%QCx(istation),turbine%blade(iblade)%QCy(istation),turbine%blade(iblade)%QCz(istation),&
                 turbine%blade_cone_angle*pi/180.0d0,0.0d0,1.0d0,0.0d0,0.0d0,0.0d0,0.d0,&
                 turbine%blade(iblade)%QCx(istation),turbine%blade(iblade)%QCy(istation),turbine%blade(iblade)%QCz(istation))
    ! Rotate tangential vectors (around y)
    call QuatRot(turbine%blade(iblade)%tx(istation),turbine%blade(iblade)%ty(istation),turbine%blade(iblade)%tz(istation),&
                 turbine%blade_cone_angle*pi/180.0d0,0.0d0,1.0d0,0.0d0,0.0d0,0.0d0,0.d0,&
                 turbine%blade(iblade)%tx(istation),turbine%blade(iblade)%ty(istation),turbine%blade(iblade)%tz(istation))
    ...
    call populate_blade_airfoils(turbine%blade(iblade)%NElem,turbine%Blade(iblade)%NAirfoilData,turbine%Blade(iblade)%EAirfoil,&
                                 turbine%Blade(iblade)%AirfoilData,turbine%Blade(iblade)%ETtoC)
    ...
    call QuatRot(turbine%RotN(1),turbine%RotN(2),turbine%RotN(3),&
                 turbine%yaw_angle*pi/180.0d0,0.0d0,1.0d0,0.0d0,0.0d0,0.0d0,0.d0,&
                 turbine%RotN(1),turbine%RotN(2),turbine%RotN(3))
    call QuatRot(turbine%RotN(1),turbine%RotN(2),turbine%RotN(3),&
                 -turbine%shaft_tilt_angle*pi/180.0d0,0.0d0,0.0d0,1.0d0,0.0d0,0.0d0,0.d0,&
                 turbine%RotN(1),turbine%RotN(2),turbine%RotN(3))
   ...
   ! ! Find the cross product Ublade = Omega x R
    ...
    allocate(turbine%ContWindSpeed(turbine%ContEntries),&
             turbine%ContOmega(turbine%ContEntries),&
