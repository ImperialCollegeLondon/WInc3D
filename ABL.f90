...
!==========================================================
subroutine abl_boundary_stresses(ux,uy,uz,taux,tauy,tauz)
...
real(mytype),dimension(xsize(1),xsize(2),xsize(3)) :: taux,tauy,tauz 
...
!*****************************************************************************************
    ...
    do k=1,xsize(3)
    do i=1,xsize(1)
    taux(i,1,k)=-u_shear**2.0*(sqrt((0.5*(ux(i,1,k)+ux(i,2,k)))**2.+(0.5*(ux(i,1,k)+ux(i,2,k)))**2.)*ux_HAve+&
                             S_HAve*(0.5*(ux(i,1,k)+ux(i,2,k))-ux_HAve))/(S_Have*sqrt(ux_HAve**2.+uz_HAve**2.))
    tauz(i,1,k)=0.
    tauz(i,1,k)=-u_shear**2.0*(sqrt((0.5*(uz(i,1,k)+uz(i,2,k)))**2.+(0.5*(uz(i,1,k)+uz(i,2,k)))**2.)*uz_HAve+&
